...
class RuntimeIntegrationsTest(unittest.TestCase):
    def test_install_runtime_integrations_patches_home_configs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                (home / ".cursor" / "bin" / "cursor_exec_guard.sh").exists(),
            )
            self.assertTrue(
                (home / ".factory" / "bin" / "factory_exec_guard.sh").exists(),
            )
            self.assertTrue((home / ".codex" / "bin" / "codex_exec_guard.sh").exists())
            self.assertTrue(
                (home / ".claude" / "bin" / "claude_pretool_guard.sh").exists(),
            )
            self.assertTrue((home / ".local" / "bin" / "codex").exists())
            self.assertTrue((home / ".local" / "bin" / "claude").exists())
            self.assertTrue(
                (home / ".local" / "bin" / "cursor.policy-federation.real").exists(),
            )
            pretool_commands = [
            ...
            self.assertIn(
                '"$HOME/.claude/bin/claude_pretool_guard.sh"', pretool_commands,
            )
            launcher_wrapper = (home / ".local" / "bin" / "cursor").read_text(
                encoding="utf-8",
            )
            self.assertIn(
                "agentops-policy-federation launcher wrapper", launcher_wrapper,
            )
            for launcher_name in ("codex", "cursor", "droid", "claude"):
                ...
                self.assertIn(
                    'export POLICY_ASK_MODE="${POLICY_ASK_MODE:-review}"', launcher,
                )
                self.assertIn(
            ...
            self.assertFalse(
                (home / ".local" / "bin" / "codex.policy-federation.real").exists(),
            )
            self.assertEqual(
    ...
    def test_install_runtime_integrations_merges_claude_hook_entry(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                len(bash_entries), 1, f"expected managed matcher entry for {matcher}",
            )
            ...
            self.assertEqual(
                len(bash_entries), 1, f"expected matcher entry for {matcher} to remain",
            )
            bash_entry = bash_entries[0]
            remaining_commands = [
                hook.get("command")
