...
class TestCrossWrapperConsistency(unittest.TestCase):
    def setUp(self):
        ...
        self.wrappers = {
            "Rust": ["cargo", "run", "--manifest-path", os.path.join(self.base_dir, "wrappers/rust/Cargo.toml"), "--"],
    ...
    def run_wrapper(self, name, command):
        cmd = self.wrappers[name] + ["--bundle", self.bundle, "--command", command, "--json", "--cwd", self.cwd]
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            print(f"DEBUG: {name} stderr: {result.stderr}")
        self.assertEqual(result.returncode, 0, f"{name} failed for command '{command}': {result.stderr}")
    ...
    def assert_consistency(self, command, expected_decision):
        ...
        for name in self.wrappers:
            ...
            self.assertEqual(verdict["decision"], expected_decision, 
        ...
        self.assertTrue(all(d == decisions[0] for d in decisions), f"Inconsistent decisions across wrappers: {results}")
