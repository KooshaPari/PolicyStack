...
def normalized_csv_rows(path: pathlib.Path, expected: list[str]) -> list[dict]:
    ...
    for row in rows:
        row["resilience_depth"] = to_int(
            row.get("resilience_depth"), "resilience_depth"
        )
        row["resilience_score"] = to_float(
            row.get("resilience_score"), "resilience_score"
        )
...
def main() -> int:
    ...
    if (
        ...
        or bool(config.get("succession_resilience_tracking_enabled", True)) is not True
    ):
    ...
    active = [
        ...
        not in {"retired", "covered", "complete"}
    ]
    low_depth = [
        row for row in active if row["resilience_depth"] < args.min_resilience_depth
    ]
    low_depth_critical = [
        row
    ...
    avg_score = (
        sum(row["resilience_score"] for row in active) / len(active) if active else 0.0
    )
