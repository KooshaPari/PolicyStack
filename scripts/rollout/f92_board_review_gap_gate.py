...
def normalized_csv_rows(path: pathlib.Path, expected: list[str]) -> list[dict]:
    ...
    for row in rows:
        row["days_since_last_review"] = to_int(
            row.get("days_since_last_review"), "days_since_last_review"
        )
...
def main() -> int:
    ...
    if (
        ...
        or bool(config.get("board_review_gap_tracking_enabled", True)) is not True
    ):
    ...
    overdue = [
        r for r in open_reviews if r["days_since_last_review"] > args.max_gap_days
    ]
    if len(open_reviews) > args.max_open_gaps or overdue:
        fail(
            f"open_gaps={len(open_reviews)} overdue_gaps={len(overdue)} "
