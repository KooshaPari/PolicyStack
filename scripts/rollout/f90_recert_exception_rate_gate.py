...
def main() -> int:
    ...
    if (
        ...
        or bool(config.get("exception_rate_tracking_enabled", True)) is not True
    ):
    ...
    open_exceptions = [
        r for r in rows if (r.get("status") or "").strip().lower() != "resolved"
    ]
    unmitigated = [
        r
        for r in open_exceptions
        if (r.get("is_mitigated") or "").strip().lower() not in {"true", "yes", "1"}
    ]
    ...
    if rate > args.max_exception_rate or stale:
        fail(
            f"open_exceptions={open_count} unmitigated_exceptions={len(unmitigated)} "
