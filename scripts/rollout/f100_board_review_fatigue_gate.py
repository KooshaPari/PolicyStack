...
def normalized_csv_rows(path: pathlib.Path, expected: list[str]) -> list[dict]:
    ...
    for row in rows:
        ...
        row["days_since_review"] = to_int(
            row.get("days_since_review"), "days_since_review"
        )
...
def main() -> int:
    ...
    if (
        ...
        or bool(board.get("board_review_fatigue_monitoring_enabled", True)) is not True
    ):
    ...
    stale = [
        row for row in active if row["days_since_review"] > args.max_review_age_days
    ]
