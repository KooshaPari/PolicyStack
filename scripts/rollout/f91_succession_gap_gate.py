...
def main() -> int:
    ...
    if (
        ...
        or bool(config.get("succession_gap_tracking_enabled", True)) is not True
    ):
    ...
    open_gaps = [
        r for r in rows if (r.get("gap_status") or "").strip().lower() != "filled"
    ]
    critical_gaps = [
        r for r in rows if (r.get("criticality") or "").strip().lower() == "critical"
    ]
    critical_open = [
        r
        for r in critical_gaps
        if (r.get("gap_status") or "").strip().lower() != "filled"
    ]
