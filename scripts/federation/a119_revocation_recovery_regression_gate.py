...
for row in rows:
    recovery_seconds = max(
        recovery_seconds,
        parse_float(
            row.get("recovery_seconds", row.get("revocation_recovery_seconds", 0.0)),
            "recovery_seconds",
        ),
    )
    recovery_rate = min(
        recovery_rate,
        parse_float(
            row.get("recovery_rate", row.get("revocation_recovery_rate", 1.0)),
            "recovery_rate",
        ),
    )
    regression_count += parse_int(
        row.get("regression_count", row.get("recovery_regressions", 0)),
        "regression_count",
    )
