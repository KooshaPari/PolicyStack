...
if isinstance(rows, dict):
    max_stall_seconds = parse_float(
        rows.get("max_stall_seconds", rows.get("stall_seconds", 0.0)),
        "max_stall_seconds",
    )
    timeout_count = parse_int(
        rows.get("timeout_count", rows.get("timeouts", 0)), "timeout_count"
    )
else:
    ...
    for row in rows:
        ...
        max_stall_seconds = max(
            max_stall_seconds,
            parse_float(
                row.get("max_stall_seconds", row.get("stall_seconds", 0.0)),
                "max_stall_seconds",
            ),
        )
        timeout_count += parse_int(
            row.get("timeout_count", row.get("timeouts", 0)), "timeout_count"
        )
