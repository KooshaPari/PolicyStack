...
if isinstance(rows, dict):
    missing_paths = to_int(
        rows.get("missing_paths", rows.get("missing_path_count", 0)), "missing_paths"
    )
    error_rate = to_float(
        rows.get("error_rate", rows.get("failure_rate", 0.0)), "error_rate"
    )
else:
    missing_paths = 0
    error_rate = 0.0
    for row in rows:
        missing_paths = max(
            missing_paths,
            to_int(
                row.get("missing_paths", row.get("missing_path_count", 0)),
                "missing_paths",
            ),
        )
        error_rate = max(
            error_rate,
            to_float(row.get("error_rate", row.get("failure_rate", 0.0)), "error_rate"),
        )
