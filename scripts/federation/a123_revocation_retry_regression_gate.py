...
for row in rows:
    retry_latency_seconds = max(
        ...
        parse_float(
            row.get(
                "retry_latency_seconds",
                row.get("revocation_retry_latency_seconds", 0.0),
            ),
            "retry_latency_seconds",
        ),
    )
    retry_success_rate = min(
        ...
        parse_float(
            row.get(
                "retry_success_rate", row.get("revocation_retry_success_rate", 1.0)
            ),
            "retry_success_rate",
        ),
    )
    retry_regression_count += parse_int(
        row.get("retry_regression_count", row.get("regression_count", 0)),
        "retry_regression_count",
    )
