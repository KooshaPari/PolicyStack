...
if isinstance(data, dict):
    ...
    for row in data:
        if not isinstance(row, dict):
            continue
        is_consistent = (
            str(row.get("consistent", row.get("is_consistent", "true"))).strip().lower()
        )
        if is_consistent not in {"1", "true", "yes", "ok", "pass", "passed"}:
            inconsistent_records += 1
        missing_records += _to_int(
            row.get("missing", row.get("missing_count", 0)), "missing"
        )
    ...
    print(
        "A100 chaos evidence payload must be a JSON object or CSV rows", file=sys.stderr
    )
    raise SystemExit(2)
