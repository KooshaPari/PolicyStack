...
if isinstance(rows, dict):
    retarget_ratio = parse_float(
        rows.get("retarget_ratio", rows.get("match_ratio", 1.0)), "retarget_ratio"
    )
    fallback_count = parse_int(
        rows.get("fallback_count", rows.get("fallbacks", 0)), "fallback_count"
    )
else:
    retarget_ratio = 1.0
    fallback_count = 0
    for row in rows:
        ...
        retarget_ratio = min(
            retarget_ratio,
            parse_float(
                row.get("retarget_ratio", row.get("match_ratio", 1.0)), "retarget_ratio"
            ),
        )
        fallback_count += parse_int(
            row.get("fallback_count", row.get("fallbacks", 0)), "fallback_count"
        )
