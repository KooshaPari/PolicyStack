...
p.add_argument("--max-backlog", type=int, default=0)
p.add_argument("--max-oldest-minutes", type=float, default=0.0)
a = p.parse_args()
m = load(a.metrics)
...
if backlog > a.max_backlog or oldest > a.max_oldest_minutes:
    print("A71 revocation backlog slo gate failed", file=sys.stderr)
    raise SystemExit(2)
