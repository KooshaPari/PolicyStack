...
def main() -> int:
    ...
    for row in records:
        ...
        window_pressure_max[window] = max(
            window_pressure_max.get(window, 0.0), pressure
        )
    ...
    for window in sorted(window_samples):
        ...
        if window_pressure_max[window] > args.max_pressure:
            fail(
                f"window={window} max_pressure={window_pressure_max[window]} > max_pressure={args.max_pressure}"
            )
