...
p.add_argument("--min-ttl-seconds", type=int, default=1)
p.add_argument("--max-expired", type=int, default=0)
a = p.parse_args()
e = load(a.evidence)
...
if ttl < a.min_ttl_seconds or expired > a.max_expired:
    print("A72 chaos evidence ttl gate failed", file=sys.stderr)
    raise SystemExit(2)
