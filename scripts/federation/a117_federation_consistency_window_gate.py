...
for row in rows:
    consistency_rate = min(
        consistency_rate,
        parse_float(
            row.get("consistency_rate", row.get("rate", 1.0)), "consistency_rate"
        ),
    )
    window_seconds = max(
        window_seconds,
        parse_float(
            row.get("window_seconds", row.get("consistency_window_seconds", 0.0)),
            "window_seconds",
        ),
    )
    window_breach_count += parse_int(
        row.get("window_breach_count", row.get("breach_count", 0)),
        "window_breach_count",
    )
