...
for row in rows:
    max_entropy_budget_window_seconds = max(
        ...
        parse_float(
            row.get(
                ...
                row.get(
                    "federation_entropy_budget_window_seconds",
    ...
    entropy_budget_window_pass_rate = min(
        ...
        parse_float(
            row.get(
                "entropy_budget_window_pass_rate",
                row.get(
                    "federation_entropy_budget_window_pass_rate",
                    row.get("window_pass_rate", 1.0),
                ),
            ),
            "entropy_budget_window_pass_rate",
        ),
    ...
    entropy_budget_window_breach_count += parse_int(
        row.get(
            "entropy_budget_window_breach_count", row.get("window_breach_count", 0)
        ),
