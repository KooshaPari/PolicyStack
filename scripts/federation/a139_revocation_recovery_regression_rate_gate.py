...
parser.add_argument(
    "--min-recovery-regression-stability-rate", type=float, default=0.97
)
...
for row in rows:
    max_recovery_regression_rate = max(
        ...
        parse_float(
            row.get(
                ...
                row.get(
                    "revocation_recovery_regression_rate",
    ...
    recovery_regression_stability_rate = min(
        ...
        parse_float(
            row.get(
                "recovery_regression_stability_rate",
                row.get(
                    "revocation_recovery_regression_stability_rate",
    ...
    recovery_regression_breach_count += parse_int(
        row.get(
            "recovery_regression_breach_count", row.get("regression_breach_count", 0)
        ),
