...
if isinstance(rows, dict):
    max_stale_age = to_float(
        rows.get("max_stale_age", rows.get("stale_age", 0.0)), "max_stale_age"
    )
    stale_ratio = to_float(
        rows.get("stale_ratio", rows.get("ratio_stale", 0.0)), "stale_ratio"
    )
else:
    max_stale_age = 0.0
    stale_ratio = 0.0
    for row in rows:
        max_stale_age = max(
            max_stale_age,
            to_float(
                row.get("max_stale_age", row.get("stale_age", 0.0)), "max_stale_age"
            ),
        )
        stale_ratio = max(
            stale_ratio,
            to_float(
                row.get("stale_ratio", row.get("ratio_stale", 0.0)), "stale_ratio"
            ),
        )
