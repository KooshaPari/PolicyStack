...
if isinstance(rows, dict):
    stability = parse_float(
        rows.get("stability_score", rows.get("score", 1.0)), "stability_score"
    )
    incident_bursts = parse_int(
        rows.get("incident_bursts", rows.get("burst_count", 0)), "incident_bursts"
    )
else:
    stability = 1.0
    incident_bursts = 0
    for row in rows:
        ...
        stability = min(
            stability,
            parse_float(
                row.get("stability_score", row.get("score", 1.0)), "stability_score"
            ),
        )
        incident_bursts = max(
            incident_bursts,
            parse_int(
                row.get("incident_bursts", row.get("burst_count", 0)), "incident_bursts"
            ),
        )
