...
if isinstance(rows, dict):
    backlog = to_int(
        rows.get("policy_backlog", rows.get("backlog", 0)), "policy_backlog"
    )
    fill_rate = to_float(
        rows.get("fill_rate", rows.get("fill_ratio", 1.0)), "fill_rate"
    )
else:
    backlog = 0
    fill_rate = 1.0
    for row in rows:
        backlog = max(
            backlog,
            to_int(row.get("policy_backlog", row.get("backlog", 0)), "policy_backlog"),
        )
        fill_rate = min(
            fill_rate,
            to_float(row.get("fill_rate", row.get("fill_ratio", 1.0)), "fill_rate"),
        )
