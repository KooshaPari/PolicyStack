...
for row in rows:
    consensus_entropy = parse_float(
        row.get("consensus_entropy", row.get("entropy", 0.0)), "consensus_entropy"
    )
    ...
    consensus_breach_count += parse_int(
        row.get("consensus_breach_count", row.get("breach_count", 0)),
        "consensus_breach_count",
    )
