...
if isinstance(rows, dict):
    handoff_ratio = parse_float(
        rows.get("handoff_ratio", rows.get("handoff_accuracy", 1.0)), "handoff_ratio"
    )
    misroute_count = parse_int(
        rows.get("misroute_count", rows.get("handoff_misroutes", 0)), "misroute_count"
    )
else:
    handoff_ratio = 1.0
    misroute_count = 0
    for row in rows:
        ...
        handoff_ratio = min(
            handoff_ratio,
            parse_float(
                row.get("handoff_ratio", row.get("handoff_accuracy", 1.0)),
                "handoff_ratio",
            ),
        )
        misroute_count += parse_int(
            row.get("misroute_count", row.get("handoff_misroutes", 0)), "misroute_count"
        )
