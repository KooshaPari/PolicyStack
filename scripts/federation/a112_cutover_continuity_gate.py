...
if isinstance(rows, dict):
    continuity = to_float(
        rows.get("continuity_ratio", rows.get("continuity", 1.0)), "continuity"
    )
    gap_count = to_int(
        rows.get("continuity_gap_count", rows.get("gap_count", 0)),
    ...
    for row in rows:
        continuity = min(
            continuity,
            to_float(
                row.get("continuity_ratio", row.get("continuity", 1.0)), "continuity"
            ),
        )
        gap_count = max(
            gap_count,
            to_int(
                row.get("continuity_gap_count", row.get("gap_count", 0)),
                "continuity_gap_count",
            ),
        )
