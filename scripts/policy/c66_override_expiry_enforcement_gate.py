...
def _row_id(r):
    return str(r.get("id") or r.get("override_id") or r.get("name") or "?")
...
as_of = _ts(a.as_of)
if not as_of:
    print("C66 invalid --as-of timestamp", file=sys.stderr)
    raise SystemExit(2)
...
for x in sorted(ov, key=lambda r: json.dumps(r, sort_keys=True)):
    if not _truthy(x.get("active", False)):
        continue
    ex = _ts(x.get("expires_at"))
    if not ex or ex <= as_of:
        b.append(_row_id(x))
for r in rows:
    if not _truthy(r.get("active", False)):
        continue
...
if b:
    print(
        "C66 override expiry enforcement breach: " + ",".join(sorted(set(b))),
        file=sys.stderr,
    )
    raise SystemExit(2)
