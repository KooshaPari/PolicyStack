...
baseline_lookup = {
    _rid(r): {
        "p95": _num(
            r.get("baseline_p95", r.get("p95_baseline", r.get("p95", 0.0))), 0.0
        ),
        "p99": _num(
            r.get("baseline_p99", r.get("p99_baseline", r.get("p99", 0.0))), 0.0
        ),
...
for row in rows:
    rid = _rid(row)
    if _truthy(row.get("drift_breach")) or str(
        row.get("status", "")
    ).strip().lower() in {"failed", "breach", "error"}:
        breached_runs += 1
        continue
...
if issues:
    print(
        "C93 replay drift baseline breach: " + "; ".join(sorted(set(issues))),
        file=sys.stderr,
    )
    raise SystemExit(2)
