...
if (
    int(metrics.get("p95_breaches", metrics.get("replay_p95_breaches", 0)))
    > a.max_p95_breaches
):
...
for row in rows:
    ...
    if _truthy(row.get("breach", False)) or status in {
        "fail",
        "failed",
        "breach",
        "timeout",
...
if breaches:
    print(
        "C81 replay latency quantile breach: " + ",".join(sorted(set(breaches))),
        file=sys.stderr,
    )
    raise SystemExit(2)
