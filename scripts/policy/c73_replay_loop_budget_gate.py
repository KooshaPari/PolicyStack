...
for item in sorted(loops, key=lambda r: json.dumps(r, sort_keys=True)):
    ...
    if (
        _num(item.get("budget_ms", item.get("loop_budget_ms", 0.0)))
        > a.max_loop_budget_ms
    ):
...
for row in rows:
    ...
    if (
        _truthy(row.get("breach"))
        or _num(row.get("budget_ms", 0.0)) > a.max_loop_budget_ms
    ):
...
if breaches:
    print(
        "C73 replay loop budget breach: " + ",".join(sorted(set(breaches))),
        file=sys.stderr,
    )
    raise SystemExit(2)
