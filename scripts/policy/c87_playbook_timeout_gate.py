...
for pdef in playbooks:
    ...
    pid = _pid(pdef)
    timeouts[pid] = _num(
        pdef.get("timeout_ms", pdef.get("timeout", a.default_timeout_ms))
    )
...
for row in rows:
    ...
    elapsed = _num(
        row.get("duration_ms", row.get("elapsed_ms", row.get("runtime_ms", 0.0)))
    )
...
if issues:
    print(
        "C87 playbook timeout breach: " + "; ".join(sorted(set(issues))),
        file=sys.stderr,
    )
    raise SystemExit(2)
