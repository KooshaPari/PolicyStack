...
p.add_argument("--max-drift-events", type=int, default=0)
a = p.parse_args()
m = json.loads(pathlib.Path(a.metrics).read_text())
rows = sorted(
    list(csv.DictReader(pathlib.Path(a.drift_csv).read_text().splitlines())),
...
if rate > a.max_drift_rate:
    b.append("drift_rate")
if events > a.max_drift_events:
    b.append("drift_events")
for r in rows:
    rr = float(r.get("drift_rate", r.get("rate", 0.0)) or 0.0)
    ev = int(r.get("drift_events", r.get("events", 0)) or 0)
    if (
        _truthy(r.get("drift_breach"))
        or _truthy(r.get("breach"))
        or rr > a.max_drift_rate
        or ev > a.max_drift_events
    ):
        b.append(f"replay:{_rid(r)}")
if b:
    print(
        "C69 replay drift budget breach: " + ",".join(sorted(set(b))), file=sys.stderr
    )
    raise SystemExit(2)
