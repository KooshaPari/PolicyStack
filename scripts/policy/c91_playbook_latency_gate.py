...
rows = sorted(
    list(csv.DictReader(pathlib.Path(a.latency_csv).read_text().splitlines())),
...
expected = sorted(
    {_pid(p) for p in (playbooks if isinstance(playbooks, list) else []) if _pid(p)}
)
...
for row in rows:
    pid = _pid(row)
    covered.add(pid)
    if _truthy(row.get("sla_breach", row.get("breach", False))) or str(
        row.get("status", "")
    ).strip().lower() in {"failed", "breach", "timeout", "error"}:
        failed += 1
        breached += 1
...
if breached > a.max_breached_runs:
    breaches.append(f"breached_run_count={breached}")
if failed > a.max_failed_runs:
    breaches.append(f"failed_run_count={failed}")

if breaches:
    print(
        "C91 playbook latency breach: " + ",".join(sorted(set(breaches))),
        file=sys.stderr,
    )
    raise SystemExit(2)
