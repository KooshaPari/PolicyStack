...
row_breaches = int(
    metrics.get("drift_quantile_breaches", metrics.get("quantile_breaches", 0.0))
)
if p95 > a.max_p95:
    breaches.append("metrics:p95")
if p99 > a.max_p99:
    breaches.append("metrics:p99")
if row_breaches > a.max_quantile_breaches:
    breaches.append("metrics:quantile_breaches")
...
for r in rows:
    ...
    if _truthy(r.get("drift_breach", r.get("breach", False))) or status in {
        "breach",
        "failed",
        "fail",
    }:
...
if breaches:
    print(
        "C89 replay drift quantile breach: " + ",".join(sorted(set(breaches))),
        file=sys.stderr,
    )
    raise SystemExit(2)
