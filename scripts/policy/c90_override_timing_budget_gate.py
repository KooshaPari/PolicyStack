...
ttl_by_id = {
    _oid(r): _num(r.get("ttl_seconds", r.get("timing_budget_seconds", 0.0)), 0.0)
    for r in rows
}
...
for item in overrides:
    ...
    budget = _num(
        item.get(
            "ttl_seconds",
            item.get("timing_budget_seconds", ttl_by_id.get(oid, a.max_ttl_seconds)),
        )
    )
    if budget <= 0:
...
for row in rows:
    if _truthy(row.get("active", True)) or str(row.get("active", "")).strip() == "":
        ...
        budget = _num(
            row.get(
                "ttl_seconds",
                row.get("timing_budget_seconds", ttl_by_id.get(rid, a.max_ttl_seconds)),
            )
        )
        if budget <= 0:
            budget = a.max_ttl_seconds
