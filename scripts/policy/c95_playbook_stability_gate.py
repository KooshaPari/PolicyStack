...
baseline_score = _num(
    stats.get("playbook_stability_score", stats.get("stability_score", 1.0)), 1.0
)
flaky_rate = _num(
    stats.get("playbook_flaky_run_rate", stats.get("flaky_rate", 0.0)), 0.0
)
...
if issues:
    print(
        "C95 playbook stability breach: " + "; ".join(sorted(set(issues))),
        file=sys.stderr,
    )
    raise SystemExit(2)
