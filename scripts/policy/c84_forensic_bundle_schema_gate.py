...
if isinstance(bundle.get("required_artifacts"), list):
    required.update(
        {str(x).strip() for x in bundle["required_artifacts"] if str(x).strip()}
    )
for artifact in (
    bundle.get("artifacts", []) if isinstance(bundle.get("artifacts"), list) else []
):
    if str(artifact.get("path", "")).strip() and _truthy(
...
if a.require_schema_version and not str(bundle.get("schema_version", "")).strip():
    print(
        "C84 forensic bundle schema gate failed: missing schema_version",
        file=sys.stderr,
    )
    raise SystemExit(2)
