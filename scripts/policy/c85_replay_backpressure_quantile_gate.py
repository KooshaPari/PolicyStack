...
stall_count = int(
    _num(metrics.get("stall_count", metrics.get("backpressure_stalls", 0.0)))
)
breach_count = int(
    _num(metrics.get("breach_count", metrics.get("backpressure_breaches", 0.0)))
)
if p95 > a.max_p95:
...
for row in rows:
    ...
    if _truthy(row.get("stalled", False)) or status in {
        "stalled",
        "stuck",
        "blocked",
        "timeout",
...
if breaches:
    print(
        "C85 replay backpressure quantile breach: " + ",".join(sorted(set(breaches))),
        file=sys.stderr,
    )
    raise SystemExit(2)
