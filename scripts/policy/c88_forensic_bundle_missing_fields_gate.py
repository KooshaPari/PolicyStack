...
def _pid(r):
    return str(
        r.get("id") or r.get("path") or r.get("artifact") or r.get("name") or "?"
    ).strip()
...
artifact_fields = {}
for artifact in (
    bundle.get("artifacts", []) if isinstance(bundle.get("artifacts"), list) else []
):
    if not isinstance(artifact, dict):
        continue
    key = str(
        artifact.get("id") or artifact.get("path") or artifact.get("artifact") or ""
    ).strip()
    if not key:
        continue
...
if issues:
    print(
        "C88 forensic bundle missing fields breach: " + "; ".join(issues),
        file=sys.stderr,
    )
    raise SystemExit(2)
