...
for pdef in playbooks:
    pid = _pid(
        {"playbook_id": pdef.get("id") or pdef.get("playbook_id") or pdef.get("name")}
    )
    if not pid:
        continue
    known_playbooks.add(pid)
    for tr in (
        pdef.get("triggers", []) if isinstance(pdef.get("triggers", []), list) else []
    ):
...
for row in rows:
    ...
    if _truthy(row.get("failed", False)) or status in {
        "failed",
        "fail",
        "error",
        "mismatch",
...
missing = sorted(
    x
    for x in required_pairs
    if x.split("::")[0] in known_playbooks and x not in seen_pairs
)
...
if issues:
    print(
        "C83 playbook trigger consistency breach: " + "; ".join(sorted(issues)),
        file=sys.stderr,
    )
    raise SystemExit(2)
